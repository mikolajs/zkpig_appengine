...
class ArticleHandler(webapp.RequestHandler):
  """Klasa obsługująca dodawanie i edycję artykułów"""
  def get(self):
    self.show_content()
  ...
  def post(self):
    ...
    if self.choice == "Zapisz":
      ...
      if self.insert:
        ...
        self.redirect('/pieceofnews?&id=%s' % (self.id) )
    else:
      if not self.insert:
        self.deletePost()
      self.redirect('/news')
  ...
  def show_content(self,  edit_info = "Dodawanie nowej wiadomości."):
    ...
    if self.canEdit():
      page_content = {'edit_info':edit_info, 'post':self.post , 'department':self.department } 
...
#przerobić tak, żeby używało insert i alter        
  ...
  def getArgum(self):
    ...
    self.department = self.request.get('department')
  ...
  def canEdit(self):
    ...
    if self.user.email() == 'mikolajsochacki@gmail.com': #admin może edytować
        return True
    if self.insert:
      authors = database.Authors.all()  
      for author in authors:
        if author.email == self.user.email(): 
          return True
      return False
    else:
      if self.post.author.email == self.user.email():
        return True
      else: 
        return False
